...
void readUserData(const string filename, HashTable &userTable, HashTable &currencyTable){
    ...
    while (getline(user_file, line)){
        ...
        userTable.insert(tokens[0], time, currencyTable.find(tokens[1]), stod(tokens[3]));   // Insert market data into the user table
...
// Function executed by each thread to process user data over time periods and write to output file
void processTimePeriodForUser(ll period, int user, HashTable userTable, HashTable currencyTable, vector<string> &results) {
    BarData bar = {0, 0, 0, 0};
    ...
    for (auto t = allTimestamps.begin(); t != allTimestamps.end(); t++) {
        ...
        auto [curr, delta] = userTable.getTransactionDelta(user, *t);
        ...
        if (bar.timestamp != curr_period) {
            if (bar.timestamp != 0) {
                output << userTable.get_key(user) << "," << fixed << setprecision(4)
    ...
    // Store the local output in the results vector
    mtx.lock();
...
void aggregateData(ll period, HashTable userTable, HashTable currencyTable, const string output_file_name) {
    ...
    for (int u = 0; u < USER_TABLE_SIZE; u++) {
        ...
        threads.emplace_back(processTimePeriodForUser, period, u, userTable, currencyTable, ref(results));
    }
...
int main(){
    ...
    for (auto& [p, file] : PERIODS){
        threads.emplace_back(aggregateData, p, userTable, currencyTable, file);
    }
